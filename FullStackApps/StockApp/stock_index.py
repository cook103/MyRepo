...
def scraped_parsed_data():
    ...
    stock_index_name = [x.text for x in stock_data.find_all("div", {"class", "ZvmM7"})]

    stock_index_price = [
        x.text for x in stock_data.find_all("span", {"class", "JLPHhb"})
    ]

    crypto_name = [x.text for x in crypto_data.find_all("div", {"class", "ZvmM7"})]

    crypto_price = [x.text for x in crypto_data.find_all("span", {"class", "JLPHhb"})]

    data_dict = {
        "stock_name": stock_index_name[:6],
        "stock_price": stock_index_price[:6],
        "crypto_name": crypto_name[:15],
        "crypto_price": crypto_price[:15],
    }
